...
namespace Code.Gameplay.Features.Statuses.Factory
{
    public class StatusFactory : IStatusFactory
    {
        ...
        public GameEntity CreateStatus(StatusSetup setup, int producerId, int targetId)
        {
            ...
            status
                .With(x => x.AddDuration(setup.Duration), setup.Duration > 0)
                .With(x => x.AddTimeLeft(setup.Duration), setup.Duration > 0)
                .With(x => x.AddPeriod(setup.Period), setup.Period > 0)
                .With(x => x.AddTimeSinceLastTick(0), setup.Period > 0)
                ;
