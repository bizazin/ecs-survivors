...
namespace Code.Gameplay.Common.Physics
{
    public class PhysicsService : IPhysicsService
    {
        ...
        {
            var hitCount = Physics2D.RaycastNonAlloc(worldPosition, direction, Hits, layerMask);
            ...
            for (var i = 0; i < hitCount; i++)
            {
                ...
                var entity = _collisionRegistry.Get<GameEntity>(hit.collider.GetInstanceID());
                if (entity == null)
                    continue;
        ...
        {
            var hitCount = Physics2D.RaycastNonAlloc(start, end, Hits, layerMask);
        ...
        {
            var hitCount = OverlapCircle(position, radius, OverlapHits, layerMask);

            DrawDebug(position, radius, 1f, Color.red);

            for (var i = 0; i < hitCount; i++)
            {
                var entity = _collisionRegistry.Get<GameEntity>(OverlapHits[i].GetInstanceID());
                if (entity == null)
                    continue;
        ...
        {
            var hitCount = Physics2D.OverlapPointNonAlloc(worldPosition, OverlapHits, layerMask);
            ...
            for (var i = 0; i < hitCount; i++)
            {
                ...
                var entity = _collisionRegistry.Get<TEntity>(hit.GetInstanceID());
                if (entity == null)
                    continue;
        ...
        {
            return Physics2D.OverlapCircleNonAlloc(worldPos, radius, hits, layerMask);
        }
